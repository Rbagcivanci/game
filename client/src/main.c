...
int initiate(Game *pGame){
    ...
    if(!(pGame->pSocket = SDLNet_UDP_Open(0))){ //0 betyder inte en server
        printf("Error: %s\n",SDLNet_GetError());
        return 0;
    }
    ...
    pGame->pBackgroundSurface = IMG_Load("C:/Users/ahmed/game/lib/resources/Pong_In_Colors_screenchot_2.3.png");
    pGame->pIpSurface = IMG_Load("C:/Users/ahmed/game/lib/resources/Pong_In_Colors_screenchot_2.3.png");
    ...
    while(!closeRequested){
        switch(pGame->state){
            case ONGOING:
                ...
                for(int i = 0; i < MAX_PADDLES; i++){
                    updatePaddlePos(pGame->pPaddle[i], deltaTime);
                    restrictPlayerWithinWindow(pGame->pPaddle[i], WINDOW_WIDTH, WINDOW_HEIGHT);
                }
...
void renderLobby(Game *pGame){
    ...
    pGame->pLobbyText = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, lobbyText, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50);

    pGame->pHostSpotText = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, hostSpotText, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 50);
    pGame->pSpot1Text = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, spot1Text, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 100);
    pGame->pSpot2Text = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, spot2Text, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 150);
    pGame->pSpot3Text = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, spot3Text, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 200);
    pGame->pSpot4Text = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, spot4Text, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 250);

    pGame->pEnterIpText = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, inputIpText, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 300);
    pGame->pOutPutIpText = createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, pGame->pIp, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 - 50 + 350);
...
void updateWithServerData(Game *pGame){
    ...
    for(int i = 0; i < MAX_PADDLES; i++){
        updatePlayerWithRecievedData(pGame->pPaddle[i], &(serverData.paddles[i]));
...
void handleInput(Game *pGame, SDL_Event *pEvent){
    ...
    if(pEvent->type == SDL_KEYDOWN){
        switch(pEvent->key.keysym.scancode){
            ...
            case SDL_SCANCODE_W:
                updatePaddleVelocityUp(pGame->pPaddle[pGame->paddleNr]);
                clientData.command = UP;
                break;
            case SDL_SCANCODE_DOWN:
            case SDL_SCANCODE_S:
                updatePaddleVelocityDown(pGame->pPaddle[pGame->paddleNr]);
                break;
        }
        memcpy(pGame->pPacket->data, &clientData, sizeof(ClientData));
        pGame->pPacket->len = sizeof(ClientData);
